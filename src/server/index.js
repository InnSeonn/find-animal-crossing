...
//취향이 비슷한 주민 가져오기
app.get('/villagers/favorite', async (req, res) => {
...
//취미 목록 가져오기
app.get('/villagers/hobby', async (req, res) => {
  res.send(['자연', '음악', '놀이', '운동', '교육', '패션']);
})

//색상 목록 가져오기
app.get('/villagers/color', async (req, res) => {
  res.send(['하늘색', '베이지색', '검정색', '파란색', '갈색', '컬러풀색', '회색', '초록색', '주황색', '분홍색', '보라색', '빨간색', '흰색', '노란색']);
})

//스타일 목록 가져오기
app.get('/villagers/style', async (req, res) => {
